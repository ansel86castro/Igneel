...
namespace Igneel.SceneComponents
{
    ...
    public class HeightField : GraphicObject<HeightField>
    {
        ...
        HeightFieldSection[,] _sections;
        LayeredMaterial[] _materials;
        TerrainSectionTester _tester = new TerrainSectionTester();        
        int _sectionVertexCount;
        int _sectionPrimitives;
        Scene _scene;
        ...
        {
            get { return _sectionVertexCount; }
        }

        public int SectionPrimitives
        {
            get { return _sectionPrimitives; }         
        }

        public HeightFieldSection[,] Sections { get { return _sections; } }

        public GraphicBuffer PositionUvBuffer { get { return _vbPosUv; } }

        public GraphicBuffer StripIndices { get { return _ibStrips; } }

        public VertexDescriptor VertDescriptor { get { return _vd; } }

        public LayeredMaterial[] Materials
        {
        ...
        {
            int count = base.SubmitGraphics(scene, node, collection);
            if (count > 0)
            {
                UpdateVisibleSections(scene.ActiveCamera);
            }
            return count;
        }
        ...
        {            
            ...
            _transform = Matrix.Invert(node.GlobalPose);
        }       
        ...
        private GraphicBuffer _CreateTriangleStrips(int xVerts, int yVerts, ushort stride)
        {
            ...
            int totalIndexesPerStrip = xVerts * 2;
            int totalIndices = totalIndexesPerStrip * totalStrips + totalStrips * 2 - 2;

            _sectionPrimitives = 2 + totalIndices - 4;
        ...
        private unsafe void _CreateGeometry(Texture2D heightMap, int nbSectionsX, int nbSectionsY)
        {
            ...
            fixed (byte* sectionVbData = vbData)
            {
                    ...
                    for (int j = 0; j < nbSectionsX; j++)
                    {
                        ...
                        for (int yOffset = 0; yOffset < yVertPerSections; yOffset++)
                        {
                            for (int xOffset = 0; xOffset < xVertPerSections; xOffset++)
                            {
                                ...
                                *(float*)(sectionVbData + offset + heightOffset) = _heights[tableIndex];
                ...
                _vbPosUv = GraphicDeviceFactory.Device.CreateVertexBuffer(dataSize, 16, data: vbData);
        ...
        private static float[] GetGaussianKernel2D(int kernelSize, float stdDev)
        {
            int i, j;
            ...
            for (i = 0; i < kernelSize; i++)
            {
                for (j = 0; j < kernelSize; j++)
                {
                    float x = j - pivot;
                    float y = i - pivot;
                    float weight = Numerics.GaussianDistribution(x, y, stdDev);
