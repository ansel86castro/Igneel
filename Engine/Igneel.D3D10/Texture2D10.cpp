...
{
	Texture2D10::Texture2D10(GraphicDevice10^ graphicDevice, ID3D10Texture2D* texture )
		:Texture2D(graphicDevice)
	{	
	...
	Texture2D10::Texture2D10(GraphicDevice10^ graphicDevice, Texture2DDesc desc, array<MappedTexture2D>^ data)
		:Texture2D(graphicDevice, desc)
	{
		_device = graphicDevice->_device;
		...
		if(data!=nullptr)
		{
			...
			SAFECALL(_device->CreateTexture2D(&td, ini, &texture));
			delete[] ini;
		}
		else
		{
			SAFECALL(_device->CreateTexture2D(&td, NULL, &texture));
		}
	...
	void Texture2D10::AddSize(D3D10_TEXTURE2D_DESC& td)
	{			
