...
{
	Texture3D10::Texture3D10(GraphicDevice10^ graphicDevice, ID3D10Texture3D* texture)
		:Texture3D(graphicDevice)
	{
		_device = graphicDevice->_device;
		_texture = texture;
		...
		if(td.BindFlags & D3D10_BIND_SHADER_RESOURCE)
		{
			...
			SAFECALL(_device->CreateShaderResourceView(texture, NULL, &rv) );
			_shaderResource = rv;		
		}		
	...
	Texture3D10::Texture3D10(GraphicDevice10^ graphicDevice, Texture3DDesc desc, array<MappedTexture3D>^ data)
		:Texture3D(graphicDevice, desc)
	{
		_device = graphicDevice->_device;
		...
		{			
			...
			SAFECALL(_device->CreateTexture3D(&td, ini, &texture));
			delete[] ini;
		}
		else
		{
			SAFECALL(_device->CreateTexture3D(&td, NULL, &texture));
		}
	...
	void Texture3D10::AddSize(D3D10_TEXTURE3D_DESC& td)
	{		
		...
		{			 			
			SAFECALL( _device->CreateTexture3D(&stagingDesc, NULL, &temp) );
