...
namespace Igneel
{
    ...
    public struct Euler : IEquatable<Euler>
    {
        ...
        public const float PIover3 = (float)Math.PI / 3;
        public const float PIover4 = (float)Math.PI / 4;
        public const float PIover6 = (float)Math.PI / 2;
        ...
        public Vector3 ToDirection()
        {
            ...
            if (Heading == 0)
            {
                ...
                d.Z = (float)Math.Cos(Pitch);
            }
            else
            {
                d.X = (float)Math.Sin(Heading);
                d.Y = -(float)Math.Sin(Pitch);
                d.Z = (float)Math.Cos(Heading);
            }
            return d;
        }
        ...
        public static float NormalizeHeading(float value)
        {
            ...
            if (floorCicles > 0)
            {                                
                value = (float)Math.Sign(value) * Numerics.TwoPI * frag;
            }
            return value;
        }
        ...
        public static Euler FromFrame(Vector3 right, Vector3 up, Vector3 front)
        {
            ...
            if (a < -0.9999f)
            {
                ...
                e.Roll = 0;
                e.Pitch = PIover2;
                e.Heading = (float)Math.Atan2(up.X, up.Z);
            }
            else if (a > 0.9999f)
            {
                e.Roll = 0;
                e.Pitch = -PIover2;
                //e.Heading = (float)Math.Atan2(up.Z, up.X);
                e.Heading = (float)Math.Atan2(up.X, up.Z);
            }
            else
            {
                ...
                if (f.X < Epsilon && f.X > -Epsilon)
                    ...
                    e.Heading = (float)Math.Atan2(f.X, f.Z);

                e.Pitch = -(float)Math.Asin(front.Y);
                //// projection of up on f
                //a = Vector3.Dot(up, f);
                //if (a < GMaths.Epsilon && a > -GMaths.Epsilon)
                //    e.Pitch = 0;
                //else
                //    e.Pitch = (float)Math.Atan2(a, up.Y);

                Vector3 u = Vector3.Cross(front, r);
                e.Roll = (float)Math.Atan2(Vector3.Dot(right, u), Vector3.Dot(right, r));
                //projection of up on r
                //a = Vector3.Dot(up, r);
                //e.Roll = -(float)Math.Atan2(a, up.Y);
            }
        ...
        public static Euler FromQuaternion(Quaternion q)
        {
            ...
            if (Math.Abs(sp) > 0.9999f)
            {
                ...
                h = (float)Math.Atan2(-x * z - w * y, 0.5f - y * y - z * z);
                b = 0.0f;
            }
            else
            {
                ...
                h = (float)Math.Atan2(x * z - w * y, 0.5f - x * x - y * y);
                b = (float)Math.Atan2(x * y - w * z, 0.5f - x * x - z * z);
            }
        ...
        public static Euler FromAxis(Vector3 right, Vector3 up, Vector3 front)
        {
            ...
            a.Pitch = (float)Math.Asin(-up.Z);
            float cosp = (float)Math.Cos(a.Pitch);
            if (cosp != 0)
            {
                a.Heading = (float)Math.Atan2(right.Z, front.Z);
                a.Roll = (float)Math.Atan2(up.X, up.Y);
            }
            else
            {
                a.Heading = (float)Math.Atan2(-front.X, right.X);
                a.Roll = 0;
            }
        ...
        {
            Euler att = new Euler();
            att.Pitch = (float)Math.Asin(direction.Y);
            ...
            else if (direction.Z < Epsilon && direction.Z > -Epsilon)
            {
                ...
                att.Heading = (float)Math.Atan2(direction.X, direction.Z);
