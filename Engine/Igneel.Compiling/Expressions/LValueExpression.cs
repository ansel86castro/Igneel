...
{   
    ...
    public class FunCallExpression : Expression
    {
        ...
        public override void CheckSemantic(Scope scope, ErrorLog log)
        {
            TypeDeclaration []argsTypes = null;
            if (Parameters != null && Parameters.Length > 0)
            {
                argsTypes = new TypeDeclaration[Parameters.Length];
                for (int i = 0; i < Parameters.Length; i++)
                {
                    Parameters[i].CheckSemantic(scope, log);                    
                    argsTypes[i] = Parameters[i].Type;
                }                
            }
            ...
            if (Left != null)
            {
                ...
                {
                    Function = type.GetFunction(FunctionName, argsTypes);                   
                }
            }
            else
            {
                Function = scope.GetFunction(FunctionName, argsTypes);                
            }
