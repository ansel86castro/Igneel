...
namespace Igneel.Compiling.Expressions
{
    public class UnaryExpression:Expression
    {
        ...
        public override void CheckSemantic(Scope scope, ErrorLog log)
        {
            ...
            {
                var primType = Expression.Type as PrimitiveTypeDeclaration;
                if (primType == null)
                    log.Error("operator not allowed", Line, Column);
                else if(primType.ReflectionType.Class != Graphics.TypeClass.Matrix &&
                    primType.ReflectionType.Class != Graphics.TypeClass.Vector &&
                    primType.ReflectionType.Class != Graphics.TypeClass.Scalar)
                {
                    log.Error("operator not allowed", Line, Column);
                }
                else if(primType == ShaderRuntime.Boolean)
                    log.Error("operator not allowed", Line, Column);
            }
        }
