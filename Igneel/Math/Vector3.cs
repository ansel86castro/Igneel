...
namespace Igneel
{
    ...
    public struct Vector3:IEquatable<Vector3>
    {
        ...
        public bool IsZero(float epsilon = Numerics.Epsilon)
        {
        ...
        public static unsafe float DistanceSquared(Vector3* a, Vector3* b)
        {
            float x = a->X - b->X;
            float y = a->Y - b->Y;
            float z = a->Z - b->Z;
        ...
        {
            Vector3 v;
            v.X = Math.Min(1, Math.Max(0, value.X));
            v.Y = Math.Min(1, Math.Max(0, value.Y));
            v.Z = Math.Min(1, Math.Max(0, value.Z));
            return v;
        }
        ...
        public static bool Equals(Vector3 a, Vector3 b, float epsilon = Numerics.Epsilon)
        {
        ...
        public static Vector3 TransformCoordinates(Vector3 position, Matrix matrix)
        {
            Vector3 vector;
            float num = (((position.X * matrix.M11) + (position.Y * matrix.M21)) + (position.Z * matrix.M31)) + matrix.M41;
            float num2 = (((position.X * matrix.M12) + (position.Y * matrix.M22)) + (position.Z * matrix.M32)) + matrix.M42;
        ...
        {
            float radius = (float)Math.Sqrt(p.X * p.X + p.Y * p.Y + p.Z * p.Z);
