...
namespace Igneel
{
    ...
    public struct Quaternion :IEquatable<Quaternion>
    {
        ...
        {
            Quaternion q;
            float a = (float)Math.Acos(quaternion.W);
            float invSin = 1.0f / (float)Math.Sin(a);
        ...
        {
            Quaternion q = a;
            if (Math.Abs(a.W) < .9999f)
            {
                // Extract the half angle alpha (alpha = theta/2)
                float alpha = (float)Math.Acos(a.W);
                // Compute new alpha value
                float newAlpha = alpha * exponent;
                // Compute new w value
                q.W = (float)Math.Acos(newAlpha);
