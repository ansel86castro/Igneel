...
{   
    public static class Service
    {
        ...
        {            
            return _Singleton<T>.Instance;
        }

        public static T GetNonNull<T>() where T : class
        {
            var instance = _Singleton<T>.Instance;
        ...
        public static void SetFactory<T>() where T : new()
        {
            ...
            _Singleton<IFactory<T>>.Instance = fac;            
        }

        public static void SetFactory<T>(IFactory<T> factory)
        {
            _Singleton<IFactory<T>>.Instance = factory;               
        }

        public static void SetFactory<T>(Func<T>func)
        {            
            _Singleton<IFactory<T>>.Instance = new DelegateFactory<T>(func);
        }

        public static IFactory<T> GetFactory<T>()
        {
            var fact =_Singleton<IFactory<T>>.Instance;
            if (fact == null)
        ...
        {
            var genType = typeof(_Singleton<>).MakeGenericType(type);
