...
namespace Igneel.Rendering
{
    ...
    {       
        ...
        {            
            if (transparent == null)
            {               
                noTransparent = Engine.Graphics.CreateBlendState(new BlendDesc(true)
                {
                    BlendEnable = false
                });

                transparent = Engine.Graphics.CreateBlendState(new BlendDesc(true)
                {
                    BlendEnable = true,
                ...
                additive =  Engine.Graphics.CreateBlendState(new BlendDesc(true)
                {
                    BlendEnable = true,
                    SrcBlend = Blend.SourceAlpha,
                    DestBlend = Blend.One
                ...
                depthStenciState = Engine.Graphics.CreateDepthStencilState(new DepthStencilStateDesc(true) 
                {
                    DepthFunc = Comparison.LessEqual
                });


                noCullingRState = Engine.Graphics.CreateRasterizerState(new RasterizerDesc(true)
                    {
                        AntialiasedLineEnable = Engine.Graphics.Info.MSAA.Count > 1,
                        MultisampleEnable = Engine.Graphics.Info.MSAA.Count > 1,
                        Cull = CullMode.None,
                    });
                defaultCullingRState = Engine.Graphics.CreateRasterizerState(new RasterizerDesc(true)
                {
                    AntialiasedLineEnable = Engine.Graphics.Info.MSAA.Count > 1,
                    MultisampleEnable = Engine.Graphics.Info.MSAA.Count > 1,
                    Cull = CullMode.Back,
    ...
    //        GEngine.Graphics.Clear(ClearFlags.Target | ClearFlags.ZBuffer | ClearFlags.Stencil, 0, 1, 0);
