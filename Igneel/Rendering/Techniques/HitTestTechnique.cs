...
namespace Igneel.Rendering
{
    ...
    {
        ReadOnlyList<IIdentificable> pickedObjects;
        Scene scene;
        ...
        {
            pickedObjects = new ReadOnlyList<IIdentificable>();
        }

       public IReadOnlyList<IIdentificable> PickedObjects { get { return pickedObjects; } }

        public Scene Scene { get { return scene; } set { scene = value; } }

        public void BeginPicking()
        {
            pickedObjects.items.Clear();
        }
        ...
        public unsafe void AddPickedIds(int* ids, int count)
        {
            ...
            Array.Sort(data);
            int lastId = -1;
            var list = pickedObjects.items;
            for (int i = 0; i < count; i++)
            {
                int id = ids[i];
                if (lastId != id)
                {
                    list.Add(scene.Identificables[i]);
                }
                lastId = id;
            }
        }
    ...
    public class HitTestTechnique: Technique
    {
        ...
        {            
            ...
            {                
                foreach (var item in Engine.Scene.GetRenderList())
                {
        ...
        public void Initialize()
        {
            ...
            blend = Engine.Graphics.CreateBlendState(new BlendDesc { BlendEnable = false, AlphaToCoverageEnable = false });
            rast = Engine.Graphics.CreateRasterizerState(new RasterizerDesc { Cull = CullMode.None, Fill = FillMode.Solid, MultisampleEnable = false, AntialiasedLineEnable = false });
