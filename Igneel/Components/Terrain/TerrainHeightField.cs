...
namespace Igneel.Components
{
    ...
    public class HeightField : GraphicObject<HeightField>
    {
        ...
        public override int  GetGraphicObjects(SceneNode node , ICollection<DrawingEntry> collection)
        {
            ...
            if (count > 0)
            {
                UpdateVisibleSections(Engine.Scene.ActiveCamera);
            }
            return count;
        }
        ...
        private GraphicBuffer _CreateTriangleStrips(int xVerts, int yVerts, ushort stride)
        {
            ...
            GraphicBuffer ib = Engine.Graphics.CreateIndexBuffer(data:indices);            
            return ib;
        }
            ...
            fixed (byte* sectionVbData = vbData)
            {
                ...
                for (int i = 0; i < nbSectionsY; i++)
                {
                    ...
                    for (int j = 0; j < nbSectionsX; j++)
                    {
                        ...
                        section.NormalHeightVb = Engine.Graphics.CreateVertexBuffer(dataSize, 24, data: vbData);
                ...
                vbPosUV = Engine.Graphics.CreateVertexBuffer(dataSize, 16, data: vbData);
    ...
    //                vBuffer = Engine.Graphics.CreateVertexBuffer<TerrainVertex>(bufferSize, Usage.WriteOnly, TerrainVertex.VertexFormat, Pool.Managed);
    //                iBuffer = Engine.Graphics.CreateIndexBuffer((bufferSize / 4) * 6, Usage.WriteOnly, Pool.Managed, true);
    ...
    //    //    vBuffer = Engine.Graphics.CreateVertexBuffer<TerrainVertex>(bufferSize, Usage.WriteOnly, TerrainVertex.VertexFormat, Pool.Managed);
    //    //    iBuffer = Engine.Graphics.CreateIndexBuffer(6 * bufferSize, Usage.WriteOnly, Pool.Managed, true);
