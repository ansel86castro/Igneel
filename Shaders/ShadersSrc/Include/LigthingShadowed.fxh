...
{	
	...
	for(int i = 0; i < NUM_SAMPLES; ++i)		
		samples[i] = (float)(tex2D(s7, gShadowTexCoord.xy + gShadowTexOffsets[i]).r > gShadowTexCoord.z);			
	
	//presample pcf values of spanded row and  expanded column
	for(int i =  loopStart; i < loopEnd; ++i)	
	{
		int index = i - loopStart;
		column[index] = (float)(tex2D(s7, gShadowTexCoord.xy + float2(  2.0f      * fTexelSize, ((float)i) * fTexelSize ) ).r > gShadowTexCoord.z);			
		row[index] 	  = (float)(tex2D(s7, gShadowTexCoord.xy + float2( ((float)i) * fTexelSize,  2.0f      * fTexelSize) ).r > gShadowTexCoord.z);				
	}	
	...
	result.w  = (float)(tex2D(s7, gShadowTexCoord.xy + float2( 2.0f * fTexelSize, 2.0f * fTexelSize) ).r > gShadowTexCoord.z);
