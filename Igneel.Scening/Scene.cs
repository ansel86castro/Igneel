...
{       
    ...
    public class Scene : IAssetProvider, IAssetProviderNotificator, IDynamic, INameable
    {
        ...
        public static void BindScene(Render render)
        {
            ...
            if (EngineState.Lighting.EnableAmbient && !ambient.IsGPUSync)
            {
                render.Bind(ambient);
        ...
        public static void UnBindScene(Render render)
        {
            var scene = SceneManager.Scene;
            if (scene.ActiveCamera != null)
                render.UnBind(scene.ActiveCamera);
